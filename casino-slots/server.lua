...
RegisterNetEvent('dc-casino:slots:server:spin', function(ChosenBetAmount)
    ...
    -- luacheck: ignore
    or UseItem and Player.Functions.RemoveItem(ItemName, SlotReferences[SlotModel].betamounts[ChosenBetAmount]) then TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['casino_chip'], "remove", ChosenBetAmount)
    ...
    SetTimeout(SpinTime, function()
        ...
        or UseItem and Player.Functions.AddItem(ItemName, RewardAmount) then 
            TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['casino_chip'], "add", RewardAmount) 
